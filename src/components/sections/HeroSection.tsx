...
export default function HeroSection() {
  ...
  return (
    <section
      className="relative min-h-[100svh] flex flex-col justify-center py-16 sm:py-24 overflow-hidden"
      style={{ background: "linear-gradient(148deg, #dcf0eb 0%, #edf5f2 28%, #e5eff5 58%, #f0ece1 100%)" }}
    >
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-12 w-full relative z-10 flex flex-col justify-center">
        <div className="max-w-3xl w-full">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 mb-8 sm:mb-10 rounded-full bg-white/75 border border-teal-400/70 backdrop-blur-sm shadow-sm"
          >
            ...
            <span className="text-[10px] font-mono tracking-[0.2em] text-teal-700 font-semibold uppercase select-none">
              Open to Work · Web3 Engineer · India
            </span>
          </motion.div>

          <div className="mb-5 sm:mb-6">
            <h1 className="leading-[0.88] tracking-tight" style={{ fontFamily: "'Georgia', 'Palatino', serif" }}>
              <div className="text-[3.6rem] sm:text-[6.5rem] md:text-[8.5rem] font-black text-gray-950 mb-2">
                {mounted ? <GlitchText text="Amit." delay={150} /> : "Amit."}
              </div>
              <div className="text-[1.3rem] sm:text-[2.2rem] md:text-[2.8rem] font-bold"
          ...
          <div className="max-w-2xl space-y-4 mb-9 sm:mb-11 relative z-20">
            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.68, duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
              className="text-[16px] sm:text-[17.5px] text-slate-700 leading-[1.82]">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }}
            className="grid grid-cols-2 sm:grid-cols-4 gap-5 sm:gap-8 pt-7 border-t border-slate-300">
            {stats.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="text-[1.75rem] sm:text-[2.4rem] font-black leading-none tabular-nums text-gray-900 group-hover:text-teal-700 transition-colors duration-300"
                  style={{ fontFamily: "'Georgia', serif" }}>
                  {mounted && !s.isStatic
