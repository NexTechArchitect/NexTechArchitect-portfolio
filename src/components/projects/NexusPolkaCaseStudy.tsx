...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Features: 5D Fluid Background, Interactive PnL Sandbox, Deep Architecture
═══════════════════════════════════════════════════════════════════ */
...
function AuroraCanvas() {
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden bg-[#FAFAFA] pointer-events-none">
...
function PnLSandbox() {
  ...
  return (
    <div className="p-6 bg-white/50 border border-white/60 rounded-2xl shadow-inner mt-4">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-sm font-black text-slate-800 uppercase tracking-widest">Live Math Engine Sandbox</h4>
      ...
      <div className="grid grid-cols-3 gap-4 border-t border-slate-200/50 pt-4">
        <div className="p-3 bg-slate-100/50 rounded-xl border border-slate-200">
          <div className="text-[10px] text-slate-500 uppercase font-bold">Position Size</div>
          <div className="text-sm sm:text-lg font-mono font-bold text-slate-800">${positionSize.toLocaleString()}</div>
        </div>
        <div className={`p-3 rounded-xl border ${pnl >= 0 ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
          <div className="text-[10px] text-slate-500 uppercase font-bold">Unrealized PnL</div>
          <div className={`text-sm sm:text-lg font-mono font-bold ${pnl >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
            {pnl >= 0 ? '+' : ''}{pnl.toFixed(2)}
          </div>
        </div>
        <div className={`p-3 rounded-xl border ${isLiquidatable ? 'bg-red-100 border-red-300' : 'bg-blue-50 border-blue-200'}`}>
          <div className="text-[10px] text-slate-500 uppercase font-bold">Total Equity</div>
          <div className={`text-sm sm:text-lg font-mono font-bold ${isLiquidatable ? 'text-red-600' : 'text-blue-600'}`}>
            ${equity.toFixed(2)}
          </div>
        </div>
      </div>
...
export default function NexusPolkaCaseStudy() {
  ...
  return (
    <div className="w-full relative min-h-screen text-slate-800 font-sans selection:bg-[#E6007A]/20 overflow-hidden rounded-[20px] md:rounded-[40px] shadow-2xl">
      ...
      <div className="relative z-10 w-full h-full flex flex-col">
          ...
          <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-10 items-center justify-between">
            ...
            <div className="flex-1 w-full text-center lg:text-left">
              <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} className="flex flex-wrap justify-center lg:justify-start gap-2 mb-6">
                <span className="px-4 py-1.5 text-[10px] font-black tracking-widest uppercase bg-white/60 border border-white backdrop-blur-md text-[#E6007A] rounded-full shadow-sm">
                  🏆 Polkadot Hackathon 2026
                </span>
                <span className="px-4 py-1.5 text-[10px] font-black tracking-widest uppercase bg-white/40 border border-white/60 backdrop-blur-md text-slate-600 rounded-full shadow-sm">
                  Asset Hub / DeFi
                </span>
              </motion.div>
              
              <motion.h1 initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.1 }}
                className="text-5xl sm:text-6xl md:text-7xl font-black tracking-tight text-slate-900 mb-6 drop-shadow-sm"
              >
                Nexus <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#E6007A] to-[#FF80BF]">Perps.</span>
              </motion.h1>
              
              <motion.p initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.2 }}
                className="text-base sm:text-lg text-slate-600 leading-relaxed max-w-xl mx-auto lg:mx-0 font-medium bg-white/30 p-4 rounded-2xl border border-white/50 backdrop-blur-sm shadow-inner"
              >
              ...
              >
                <Link href="https://nexus-protocol-v2.vercel.app/" target="_blank"
                  className="w-full sm:w-auto px-8 py-4 text-xs font-black uppercase tracking-widest rounded-2xl bg-[#E6007A] text-white hover:bg-[#C20067] hover:shadow-[0_10px_40px_rgba(230,0,122,0.3)] hover:-translate-y-1 transition-all text-center">
                  Launch App ⚡
                </Link>
                <Link href="https://github.com/NexTechArchitect/nexus-polka-perps" target="_blank"
                  className="w-full sm:w-auto px-8 py-4 text-xs font-black uppercase tracking-widest rounded-2xl bg-white/70 border border-white backdrop-blur-md text-slate-700 hover:text-[#E6007A] hover:bg-white shadow-sm hover:shadow-md transition-all text-center">
                  Source Code ↗
                </Link>
              </motion.div>
            </div>
        ...
        <div className="sticky top-0 z-40 bg-white/50 backdrop-blur-2xl border-b border-white/60 shadow-sm">
          <div className="max-w-6xl mx-auto px-5 py-2 flex gap-2 md:gap-4 overflow-x-auto scrollbar-hide items-center h-16">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`shrink-0 relative px-4 md:px-6 py-2.5 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-xl transition-all ${
                  tab === t.id 
        ...
        {/* ── DEEP CONTENT AREA ── */}
        <div className="w-full max-w-6xl mx-auto px-5 sm:px-10 py-10 pb-24 h-auto min-h-[60vh]">
          <AnimatePresence mode="wait">
            
            {/* ════ OVERVIEW ════ */}
            {tab === "overview" && (
              <motion.div key="ov" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <GlassCard className="p-8 md:col-span-2">
                  ...
                  <h2 className="text-2xl sm:text-4xl font-black text-slate-900 mb-4">Redefining On-Chain Perpetuals.</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {[
                      ...
                      <div key={i} className="p-4 bg-white/60 border border-white rounded-2xl shadow-sm">
                        <div className="text-xs font-black text-slate-800 uppercase mb-2">{item.t}</div>
                ...
                <GlassCard className="p-8">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-red-50/50 border border-red-100 rounded-2xl">
                      <h4 className="text-red-600 text-xs font-black uppercase mb-1 line-through">Dust Sweep / Drain Attacks</h4>
                      <p className="text-slate-600 text-xs leading-relaxed">
                        Protocols using USDC (6 dec) often suffer from dust sweeping during division.
                      </p>
                    </div>
                    <div className="p-4 bg-emerald-50/50 border border-emerald-100 rounded-2xl">
                      <h4 className="text-emerald-600 text-xs font-black uppercase mb-1">18-Decimal Normalization</h4>
                      <p className="text-slate-600 text-xs leading-relaxed">
                ...
                <GlassCard className="p-8">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-red-50/50 border border-red-100 rounded-2xl">
                      <h4 className="text-red-600 text-xs font-black uppercase mb-1 line-through">ERC-4626 Share Inflation</h4>
                      <p className="text-slate-600 text-xs leading-relaxed">
                        First depositor manipulating the exchange rate by sending direct asset transfers to the vault.
                      </p>
                    </div>
                    <div className="p-4 bg-emerald-50/50 border border-emerald-100 rounded-2xl">
                      <h4 className="text-emerald-600 text-xs font-black uppercase mb-1">Permanent Burn Genesis</h4>
                      <p className="text-slate-600 text-xs leading-relaxed">
              ...
              <motion.div key="arch" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <GlassCard className="p-8">
                    ...
                    <div className="relative pl-6 border-l-2 border-[#E6007A]/20 space-y-6">
                      {[
                        ...
                        <div key={i} className="relative">
                          <div className="absolute -left-[31px] top-1 w-4 h-4 bg-white border-2 border-[#E6007A] rounded-full" />
                          <h4 className="text-sm font-black text-slate-800">{node.title}</h4>
                          <p className="text-[10px] font-mono text-[#E6007A] my-1">{node.sub}</p>
                  ...
                  <div className="flex flex-col h-full gap-6">
                    <GlassCard className="p-8 flex-1">
                      ...
                      <h2 className="text-2xl font-black text-slate-900 mb-2">PnLCalculator.sol</h2>
                      <p className="text-xs text-slate-600 mb-0">Pure Solidity library. Zero state. Interactive demonstration below.</p>
                ...
                <GlassCard className="p-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={i} className="p-5 bg-white/50 border border-white/60 rounded-2xl hover:border-[#E6007A]/30 transition-all shadow-sm">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="text-xs font-black text-rose-600 line-through opacity-70">{sec.a}</h4>
                          <span className="px-2 py-0.5 bg-slate-800 text-white text-[9px] font-black uppercase rounded">{sec.lvl}</span>
                        </div>
                        <p className="text-xs text-slate-700 leading-relaxed font-medium">{sec.f}</p>
                      </div>
                    ))}
                  </div>
                </GlassCard>

                <GlassCard className="p-6 flex items-center gap-4 border-amber-200/50 bg-amber-50/50">
                  <div className="text-3xl">⚠️</div>
                  <div>
                    <h4 className="text-xs font-black text-amber-700 uppercase tracking-widest mb-1">No Formal Audit</h4>
                    <p className="text-xs text-amber-900/80 font-medium">This protocol was built for the Polkadot Solidity Hackathon 2026. It is deployed on the Testnet and uses mock assets. It has not undergone formal verification by external auditors.</p>
                  </div>
                </GlassCard>
            ...
            {tab === "tests" && (
              <motion.div key="ts" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  ...
                  {/* Terminal Trace */}
                  <GlassCard className="p-0 overflow-hidden lg:col-span-3 flex flex-col h-[500px]">
                    <div className="px-4 py-3 bg-white/60 border-b border-white/60 flex items-center gap-2">
                      <div className="flex gap-1.5"><span className="w-3 h-3 rounded-full bg-rose-500"/><span className="w-3 h-3 rounded-full bg-amber-400"/><span className="w-3 h-3 rounded-full bg-emerald-400"/></div>
                      <span className="ml-3 text-[10px] font-mono font-bold text-slate-500">forge test -vvv --match-contract NexusPolka</span>
                    </div>
                    <div className="p-6 bg-slate-900 flex-1 overflow-y-auto text-xs font-mono leading-loose shadow-inner custom-scrollbar">
                      ...
                      {[
                        ...
                        <div key={i} className="flex justify-between items-center hover:bg-white/5 px-2 py-1 rounded">
                          <span className="text-emerald-400">[PASS] <span className="text-slate-300">{test.n}</span></span>
                          <span className="text-[9px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded">{test.t}</span>
                        </div>
                      ))}
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-4">
                    <GlassCard className="p-6">
                      ...
                      <p className="text-xs text-slate-500 font-medium mb-4">Chain ID: 420420417</p>
