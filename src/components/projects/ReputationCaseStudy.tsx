...
/* ═══════════════════════════════════════════════════════════════════
   ...
   Aesthetic: Fluid Mesh Gradient (Video-like), Frosted Glass, Luxury Web3
   Colors: Rose Gold, Soft Lilac, Champagne, Ice Blue
═══════════════════════════════════════════════════════════════════ */
...
function CinematicFluidCanvas() {
  ...
  useEffect(() => {
    ...
    // Mixing Light Theme Colors: Rose Gold, Lilac, Champagne, Mint/Ice
    const colors = [
    ...
    const draw = () => {
      ...
      colors.forEach((color, i) => {
        // Complex figure-8 and sweeping movements to look like fluid video
  ...
  return (
    <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden rounded-[inherit]">
      ...
      {/* Glossy overlay to give it a screen/video feel */}
...
function SoulboundVisualizer() {
  ...
  return (
    <div className="relative w-full aspect-square max-w-[340px] mx-auto flex items-center justify-center perspective-[1200px]">
      <motion.div 
      ...
      >
        ...
        <motion.div style={{ translateZ: 20 }} className="absolute inset-8 rounded-full bg-white/60 backdrop-blur-xl border-[4px] border-[#B76E79]/30 shadow-[0_30px_60px_rgba(0,0,0,0.08),inset_0_0_30px_rgba(183,110,121,0.2)] flex items-center justify-center overflow-hidden">
          <div className="text-center">
            <div className="l-serif text-7xl text-[#B76E79] mb-1 drop-shadow-md">🥇</div>
            <div className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-500">Soulbound</div>
            <div className="text-xl font-bold text-slate-900 mt-1">Gold Tier</div>
          </div>
        </motion.div>
...
function TierShowcase() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-10 items-center bg-white/30 border border-white/80 rounded-3xl p-6 md:p-10 shadow-[0_10px_40px_rgba(0,0,0,0.03)] backdrop-blur-md">
      <div className="w-48 h-48 sm:w-64 sm:h-64 relative flex items-center justify-center shrink-0">
        <AnimatePresence mode="popLayout">
          ...
          >
            <div className="w-40 h-40 sm:w-56 sm:h-56 rounded-full flex items-center justify-center bg-white/80 shadow-[0_20px_50px_rgba(0,0,0,0.08)] border-[4px] relative"
                 style={{ borderColor: t.color }}>
              <span className="text-7xl sm:text-8xl drop-shadow-lg">{t.icon}</span>
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div className="flex-1 w-full space-y-8">
        <div className="flex gap-2 overflow-x-auto hide-scroll pb-2">
          {TIERS.map((tier, i) => (
            <button key={i} onClick={() => setActive(i)}
              className={`px-4 py-2 rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-widest transition-all border ${
        ...
        <AnimatePresence mode="wait">
          <motion.div key={t.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-5">
            <h3 className="l-serif text-3xl sm:text-4xl font-bold" style={{ color: t.color }}>{t.name} Protocol Tier</h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-slate-200/50 pt-5">
              <div className="p-4 bg-white/60 rounded-2xl border border-white">
                <p className="text-[9px] text-slate-400 font-mono uppercase mb-1">Score Requirement</p>
                <p className="text-lg font-black text-slate-800">{t.score}</p>
              </div>
              <div className="p-4 bg-white/60 rounded-2xl border border-white">
                <p className="text-[9px] text-slate-400 font-mono uppercase mb-1">Governance Multiplier</p>
                <p className="text-lg font-black text-slate-800">{t.pwr}</p>
              </div>
              <div className="p-4 bg-white/60 rounded-2xl border border-white">
                <p className="text-[9px] text-slate-400 font-mono uppercase mb-1">DeFi Margin Limit</p>
                <p className="text-lg font-black text-emerald-600">{t.loan}</p>
              </div>
            </div>
            
            <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/50 flex items-start gap-3">
...
function AddressChip({ label, address }: { label: string; address: string; }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white/60 border border-white rounded-2xl hover:border-[#9B88ED]/40 hover:bg-white/90 transition-all group shadow-sm">
      <div className="flex flex-col mb-2 sm:mb-0">
        <span className="text-[9px] text-slate-400 l-mono uppercase tracking-widest">{label}</span>
        <span className="text-xs sm:text-sm text-slate-800 l-mono mt-0.5 font-bold group-hover:text-[#9B88ED] transition-colors">{short}</span>
      </div>
      <div className="flex gap-2 w-full sm:w-auto">
        <button onClick={copy} className="flex-1 sm:flex-none py-2 px-3 rounded-xl bg-slate-100 text-slate-500 hover:text-slate-900 hover:bg-slate-200 transition-colors text-[10px] font-bold uppercase tracking-widest">
          {copied ? "Copied" : "Copy"}
        </button>
        <Link href={`https://sepolia.etherscan.io/address/${address}`} target="_blank" className="flex-1 sm:flex-none py-2 px-3 rounded-xl bg-[#9B88ED]/10 text-[#9B88ED] hover:bg-[#9B88ED] hover:text-white transition-colors text-[10px] font-bold uppercase tracking-widest text-center">
          Etherscan ↗
        </Link>
      </div>
    </div>
...
export default function ReputationCaseStudy() {
  ...
  return (
    <>
      <style>{STYLES}</style>
      <div className="l-root w-full relative h-auto min-h-screen overflow-hidden rounded-[20px] md:rounded-[40px] selection:bg-[#B76E79]/20 selection:text-[#B76E79] flex flex-col">
        
        {/* Animated Fluid Canvas */}
        <CinematicFluidCanvas />

        {/* ── HERO SECTION ── */}
        <div className="w-full px-5 sm:px-10 pt-16 sm:pt-24 pb-12 sm:pb-20 relative z-10">
          <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-12 items-center">
            
            <div className="flex-1 w-full text-center lg:text-left">
              <Reveal direction="up" className="flex flex-wrap justify-center lg:justify-start gap-2 sm:gap-3 mb-6">
                <span className="px-4 py-1.5 text-[9px] sm:text-[10px] font-bold tracking-widest uppercase border border-[#B76E79]/30 bg-[#B76E79]/10 text-[#B76E79] rounded-full shadow-sm">
                  🛡️ ERC-5484 Protocol
                </span>
                <span className="px-4 py-1.5 text-[9px] sm:text-[10px] font-bold tracking-widest uppercase border border-[#9B88ED]/30 bg-[#9B88ED]/10 text-[#9B88ED] rounded-full shadow-sm">
                  Identity & Reputation
                </span>
              </Reveal>
              
              <Reveal direction="up" delay={0.1} className="mb-6">
                <h1 className="l-serif text-[42px] sm:text-6xl md:text-[80px] font-bold tracking-tight text-slate-900 leading-[1.05]">
                  Unforgeable <br/>
              ...
              <Reveal direction="up" delay={0.3} className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8 justify-center lg:justify-start">
                <Link href="https://rst-reputation-protocol.vercel.app/" target="_blank"
                  className="px-8 py-4 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-2xl bg-slate-900 text-white hover:bg-[#B76E79] hover:shadow-[0_10px_30px_rgba(183,110,121,0.3)] hover:-translate-y-1 transition-all text-center">
                  Live Dashboard ↗
                </Link>
                <Link href="https://github.com/NexTechArchitect/RST-Reputation-Protocol" target="_blank"
                  className="px-8 py-4 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-2xl border border-slate-200 text-slate-600 bg-white/60 hover:border-[#9B88ED]/50 hover:bg-white hover:text-[#9B88ED] transition-all text-center shadow-sm">
                  Source Code
                </Link>
              </Reveal>
            </div>
            ...
            <Reveal direction="left" delay={0.2} className="flex-1 w-full max-w-[340px] sm:max-w-md lg:max-w-lg mt-8 lg:mt-0 relative z-20">
              <SoulboundVisualizer />
            </Reveal>
        ...
        <div className="sticky top-0 z-40 bg-white/60 backdrop-blur-2xl border-y border-slate-200/50 shadow-sm">
          <div className="max-w-6xl mx-auto px-5 flex gap-2 sm:gap-6 overflow-x-auto scrollbar-hide items-center h-14 sm:h-16">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)}
                className={`shrink-0 relative px-3 sm:px-4 py-4 text-[9px] sm:text-[10px] font-black uppercase tracking-widest transition-all ${
        ...
        {/* ── CONTENT AREA ── */}
        <div ref={scrollRef} className="w-full max-w-6xl mx-auto px-5 sm:px-10 py-12 pb-24 relative z-10 h-auto flex-1">
          <AnimatePresence mode="wait">
            
            {/* ════ PHILOSOPHY ════ */}
            {tab === "philosophy" && (
              <motion.div key="ph" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.4}} className="space-y-6 sm:space-y-8">
                
                <GlassCard className="p-8 sm:p-12">
                  <h3 className="text-[#B76E79] text-[10px] font-black uppercase tracking-widest mb-4">The Problem Statement</h3>
                  <h2 className="l-serif text-3xl sm:text-5xl font-bold text-slate-900 mb-6 leading-tight">Anonymous Capital vs <br className="hidden sm:block"/>Proven Trust.</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                  {[
                    ...
                    <GlassCard key={i} className="p-6 sm:p-8 flex flex-col justify-center text-center items-center hover:-translate-y-1 transition-transform">
                      <div className="w-12 h-12 rounded-full border border-[#9B88ED]/30 bg-[#9B88ED]/10 flex items-center justify-center text-[#9B88ED] mb-4 font-black l-serif text-lg shadow-sm">0{i+1}</div>
                      <h4 className="text-xs font-black text-slate-800 mb-3 uppercase tracking-widest">{item.t}</h4>
                      <p className="text-[11px] sm:text-xs text-slate-500 leading-relaxed">{item.d}</p>
                    </GlassCard>
                  ))}
                </div>
              </motion.div>
            ...
            {/* ════ ENGINE & MECHANICS ════ */}
            {tab === "mechanics" && (
              <motion.div key="mech" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.4}} className="space-y-6 sm:space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Left: Scoring Actions */}
                  <GlassCard className="p-6 sm:p-8">
                    <h3 className="text-[#B76E79] text-[10px] font-black uppercase tracking-widest mb-6">Action Scoring Matrix</h3>
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={i} className="flex justify-between items-center p-4 bg-white/60 border border-white/80 rounded-2xl shadow-sm hover:bg-white/90 transition-colors">
                          <div>
                            <p className="text-[10px] sm:text-xs font-black text-slate-800 uppercase tracking-wider">{act.a}</p>
                            <p className="text-[9px] sm:text-[10px] l-mono text-slate-400 mt-1">{act.c}</p>
                          </div>
                          <span className={`text-sm sm:text-lg font-black l-mono ${act.bad ? 'text-rose-500' : 'text-emerald-500'}`}>{act.p}</span>
                        </div>
                      ))}
                    </div>
                  </GlassCard>
                  ...
                  <GlassCard className="p-6 sm:p-8 flex flex-col">
                    <h3 className="text-[#9B88ED] text-[10px] font-black uppercase tracking-widest mb-4">ReputationMath.sol</h3>
                    <h2 className="l-serif text-2xl sm:text-3xl font-bold text-slate-900 mb-4">Strict Clamping.</h2>
                    <p className="text-xs sm:text-sm text-slate-600 leading-relaxed font-medium mb-6">
                      The core math library ensures the score never breaches the bounds of <code className="text-[#9B88ED] bg-[#9B88ED]/10 px-1 py-0.5 rounded l-mono text-xs border border-[#9B88ED]/20">[0, 1000]</code>. Underflows from massive defaults are caught and floored to 0. Overflows ceiling at 1000.
                    </p>
                    <div className="flex-1 p-4 sm:p-5 bg-slate-900 border border-slate-800 rounded-2xl l-mono text-[9px] sm:text-[10px] text-slate-300 overflow-x-auto shadow-inner leading-loose">
            ...
            {/* ════ ART & SVG ════ */}
            {tab === "art" && (
              <motion.div key="art" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.4}} className="space-y-8">
                <div className="text-center max-w-2xl mx-auto mb-8">
                  <h3 className="text-[#B76E79] text-[10px] font-black uppercase tracking-widest mb-3">Dynamically Injected SVGs</h3>
                  <h2 className="l-serif text-3xl sm:text-4xl font-bold text-slate-900 mb-4">Art via Mathematics.</h2>
            ...
            {/* ════ SECURITY ════ */}
            {tab === "security" && (
              <motion.div key="sec" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.4}} className="space-y-6">
                <GlassCard className="p-6 sm:p-8">
                  <h3 className="text-[#9B88ED] text-[10px] font-black uppercase tracking-widest mb-6">Security Invariants</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={i} className="p-5 bg-white/60 border border-white/80 rounded-2xl shadow-sm hover:border-[#9B88ED]/40 hover:bg-white/90 transition-colors">
                        <div className="flex justify-between items-start mb-3 gap-2">
                          <h4 className="text-[10px] sm:text-xs font-black text-slate-800 uppercase tracking-widest leading-relaxed">{sec.a}</h4>
                          <span className="px-2 py-0.5 bg-[#9B88ED]/10 text-[#9B88ED] border border-[#9B88ED]/20 text-[8px] font-black uppercase rounded shrink-0">{sec.lvl}</span>
                        </div>
                        <p className="text-[11px] sm:text-xs text-slate-600 leading-relaxed">{sec.f}</p>
                      </div>
                    ))}
                  </div>
                </GlassCard>
            ...
            {/* ════ DEPLOYMENT ════ */}
            {tab === "deployment" && (
              <motion.div key="dep" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.4}} className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                  {/* Foundry Terminal */}
                  <GlassCard className="lg:col-span-3 p-0 overflow-hidden flex flex-col h-[400px] sm:h-[500px]">
                    <div className="px-4 sm:px-5 py-3 sm:py-4 bg-white/80 border-b border-slate-200 flex items-center gap-3 backdrop-blur-md">
                      ...
                      <span className="l-mono text-[9px] sm:text-[10px] text-slate-500 truncate">forge test -vvv --match-path test/integration/*</span>
                    </div>
                    <div className="p-4 sm:p-6 bg-slate-900 flex-1 overflow-y-auto text-[10px] sm:text-xs l-mono leading-loose shadow-inner text-slate-400 custom-scrollbar">
                      ...
                      <p className="text-slate-500 my-2">... 54 unit & fuzz tests ...</p>
                      <br/>
                      ...
                      <p className="mt-4 text-slate-500 italic">Note: Real execution logs may vary based on your local Foundry environment.</p>
                    </div>
                  </GlassCard>
                  ...
                  <div className="lg:col-span-2 flex flex-col gap-4">
                    <GlassCard className="p-6 sm:p-8">
                      <h3 className="text-[#B76E79] text-[10px] font-black uppercase tracking-widest mb-1">Sepolia Testnet Mocks</h3>
                    ...
                    <GlassCard className="p-5 flex items-center gap-4 bg-rose-50/50 border-rose-100">
                      <span className="text-2xl sm:text-3xl shrink-0">🔒</span>
                      <div>
                        <p className="text-[9px] sm:text-[10px] font-black text-rose-600 uppercase tracking-widest mb-1">Transfer Locked</p>
                        <p className="text-[10px] sm:text-[11px] text-rose-800/70 font-medium leading-relaxed">It is mathematically impossible to sell or trade your reputation.</p>
                      </div>
                    </GlassCard>
                  </div>
                </div>
