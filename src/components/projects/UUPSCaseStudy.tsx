...
function MemoryGridCanvas() {
  ...
  useEffect(() => {
    ...
    const packets: Packet[] = Array.from({ length: isMobile ? 12 : 25 }, () => ({
    ...
    const draw = () => {
      ...
      ctx.strokeStyle = "rgba(184,115,51,0.06)"; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = "rgba(184,115,51,0.15)";
      for (let x = 0; x <= W; x += spacing) for (let y = 0; y <= H; y += spacing) ctx.fillRect(x - 1, y - 1, 3, 3);
      packets.forEach(p => {
        if (time > p.delay) {
          if (p.vertical) { p.y += p.speed; if (p.y > H + 20) p.y = -20; if (p.y < -20) p.y = H + 20; }
          else { p.x += p.speed; if (p.x > W + 20) p.x = -20; if (p.x < -20) p.x = W + 20; }
          const grad = ctx.createLinearGradient(p.vertical ? p.x : p.x-(p.speed*20), p.vertical ? p.y-(p.speed*20) : p.y, p.x, p.y);
          grad.addColorStop(0, "rgba(184,115,51,0)"); grad.addColorStop(1, "rgba(184,115,51,0.6)");
          ctx.beginPath();
          if (p.vertical) { ctx.moveTo(p.x, p.y-(p.speed*20)); ctx.lineTo(p.x, p.y); }
          else { ctx.moveTo(p.x-(p.speed*20), p.y); ctx.lineTo(p.x, p.y); }
          ctx.strokeStyle = grad; ctx.lineWidth = 2; ctx.stroke();
          ctx.beginPath(); ctx.arc(p.x, p.y, 2.5, 0, Math.PI*2); ctx.fillStyle = "rgba(210,130,50,0.9)"; ctx.fill();
        }
      });
...
function SlotCanvas() {
  ...
  useEffect(() => {
    ...
    const SLOTS = [
      { n:"feeRate", type:"uint256", kind:"data" }, { n:"owner", type:"address", kind:"data" },
      { n:"paused", type:"bool", kind:"data" }, { n:"version", type:"uint8", kind:"special" },
      { n:"configHash", type:"bytes32", kind:"special" }, { n:"__gap[0]", type:"uint256", kind:"gap" },
      { n:"__gap[1]", type:"uint256", kind:"gap" }, { n:"...", type:"...", kind:"gap" },
      { n:"__gap[49]", type:"uint256", kind:"gap" },
    ];
    const draw = () => {
      const W = c.width, H = c.height; ctx.clearRect(0,0,W,H); t+=0.022;
      const isMobile = W < 400;
      const sh = isMobile ? 24 : 28, sg = isMobile ? 4 : 6, pad = isMobile ? 10 : 16;
      ...
      for (let i=0;i<count;i++) {
        ...
        ctx.fillStyle=isGap?`rgba(184,115,51,${0.07+pulse*0.05})`:isSpec?`rgba(184,115,51,${0.09+pulse*0.06})`:`rgba(28,26,23,${0.04+pulse*0.02})`; ctx.fill();
        ...
        ctx.beginPath(); ctx.roundRect(pad,y,bw,sh,6); ctx.fillStyle=isGap?"rgba(184,115,51,0.09)":"rgba(28,26,23,0.04)"; ctx.fill();
        const fontSize=isMobile?9:10, textY=y+(sh/2)+3;
  ...
  return (
    <>
      ...
      <div className="u-root w-full overflow-x-hidden bg-[#FAF7F0] text-[#1C1A17] selection:bg-[#B87333]/20 selection:text-[#5A3018]">
        ...
        {/* ── HERO — content-driven height, no giant canvas gap ── */}
        <div className="relative w-full bg-[#FAF7F0] overflow-hidden border-b border-[#B87333]/20 flex flex-col justify-end"
          style={{ paddingTop: "clamp(48px, 8vw, 100px)" }}>
          
          {/* Canvas capped height so it doesn't dominate mobile */}
          ...
          <div className="relative z-10 px-5 md:px-12 pb-8 md:pb-12 w-full max-w-7xl mx-auto">
            <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} className="flex flex-wrap gap-2 mb-3 md:mb-6">
            ...
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.1}}>
              <div className="u-label mb-2 text-[9px] md:text-[10px] font-bold text-[#A08060]">Smart Contract Architecture</div>
              <h1 className="u-serif text-4xl sm:text-5xl md:text-7xl font-bold leading-[1.1] text-[#1C1A17] mb-3 md:mb-6">
              ...
              <p className="text-xs md:text-base leading-relaxed text-[#6B5030] max-w-2xl font-medium border-l-2 border-[#B87333] pl-3 md:pl-5">
                A production-ready implementation of the Universal Upgradeable Proxy Standard. Demonstrates storage layout safety, atomic state migrations, and gas-efficient delegation patterns.
              </p>
            </motion.div>
            <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.2}} className="flex flex-wrap gap-4 mt-5 md:mt-8">
        ...
        <div className="border-b bg-white" style={{borderColor:"rgba(184,115,51,0.15)"}}>
          <div className="max-w-7xl mx-auto grid grid-cols-4 divide-x divide-[#B87333]/10 py-4 md:py-0">
            {[
              { v: "V1→V3", sub: "Lifecycle", note: "3 stages" },
              { v: "EIP-1967", sub: "Slot Std", note: "Collision-proof" },
              { v: "50", sub: "__gap", note: "Future-proofed" },
              { v: "0 Admin", sub: "Overhead", note: "vs Transparent" },
            ].map((s,i) => (
              <div key={i} className="px-2 md:p-8 py-4 text-left">
                <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.2+i*0.05}}>
                  <div className="u-serif text-lg sm:text-2xl md:text-4xl font-bold text-[#B87333] mb-1 leading-none truncate">{s.v}</div>
                  <div className="u-label text-[7px] md:text-[10px] text-[#7A3A12] font-bold truncate">{s.sub}</div>
                  <div className="text-[7px] md:text-[10px] text-[#A08060] hidden sm:block">{s.note}</div>
                </motion.div>
              </div>
            ))}
          </div>
          ...
          <div className="max-w-7xl mx-auto px-4 md:px-12 pt-2 md:pt-4 pb-0 flex overflow-x-auto hide-scroll gap-1 md:gap-4">
            {TABS.map(t => (
              <button key={t.id} onClick={()=>setTab(t.id)}
                ...
                {t.label}
                {tab===t.id && <motion.div layoutId="uups-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#B87333]" />}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-12 py-6 md:py-16">
          <AnimatePresence mode="wait">
              ...
              <motion.div key="ov" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-5 md:space-y-8">
                      ...
                      <tbody className="divide-y divide-[#B87333]/10">
                        {[
                          ...
                          <tr key={i} className="hover:bg-[#FAF7F0]/50 transition-colors">
                            <td className="px-4 md:px-6 py-3 text-xs md:text-sm font-bold text-[#1C1A17] whitespace-nowrap">{row.f}</td>
                            <td className="px-4 md:px-6 py-3 text-[10px] md:text-sm text-[#A08060]">{row.e}</td>
                            <td className="px-4 md:px-6 py-3 text-[10px] md:text-sm text-[#7A3A12] font-semibold">{row.a}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <TiltCard intensity={3} className="p-5 md:p-8 rounded-xl md:rounded-2xl border bg-white shadow-sm" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                    <div className="u-label mb-2 text-[9px] font-bold" style={{color:"#B09070"}}>// Why UUPS</div>
                    <h3 className="text-base md:text-lg font-bold text-[#1C1A17] mb-2">Eliminating Admin Checks</h3>
                    <p className="text-xs md:text-sm leading-relaxed text-[#6B5030]">
                      Transparent Proxies run an admin check on every user call. UUPS moves <code className="u-mono bg-[#FAF7F0] px-1.5 py-0.5 rounded text-[#7A3A12] border border-[#B87333]/20">upgradeToAndCall</code> into the implementation itself — zero overhead on standard transactions.
                    </p>
                  </TiltCard>
                  <TiltCard intensity={3} className="p-5 md:p-8 rounded-xl md:rounded-2xl border bg-white shadow-sm" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                    <div className="u-label mb-3 text-[9px] font-bold" style={{color:"#B09070"}}>// Repository Scope</div>
                    <div className="space-y-3">
                      {[
                        ...
                        <div key={i} className="flex items-start gap-3">
                          <span className="u-mono w-5 h-5 shrink-0 rounded flex items-center justify-center font-bold text-[10px]" style={{background:"rgba(184,115,51,0.1)",color:"#7A3A12"}}>{i+1}</span>
                          <div>
                            <h4 className="text-xs md:text-sm font-bold text-[#1C1A17]">{item.title}</h4>
                            <p className="text-[10px] md:text-xs text-[#A08060] mt-0.5">{item.desc}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {tab==="architecture" && (
              <motion.div key="arch" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-5 md:space-y-8">
                <h3 className="u-label text-[10px] md:text-xs font-bold text-[#A08060] mb-4 text-center md:text-left">The DelegateCall Flow</h3>
                <div className="rounded-xl md:rounded-3xl border p-5 md:p-12 bg-white shadow-sm flex flex-col lg:flex-row items-center justify-between gap-6 md:gap-12" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                  <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 w-full lg:w-auto">
                    <TiltCard intensity={5} className="rounded-lg md:rounded-xl border p-4 md:p-6 text-center w-full md:w-32 shadow-sm shrink-0" style={{background:"#FDF9F3",borderColor:"rgba(28,26,23,0.1)"}}>
                      ...
                      <div className="u-label text-[9px] font-bold text-[#A08060]">User</div>
                    </TiltCard>
                    <div className="flex flex-col md:flex-row items-center gap-1.5">
                      ...
                      <span className="u-mono px-2 py-1 rounded bg-[#B87333]/10 text-[9px] font-bold text-[#7A3A12]">call()</span>
                    ...
                    <TiltCard intensity={5} className="rounded-lg md:rounded-xl border p-5 md:p-8 w-full md:w-56 shrink-0 relative overflow-hidden"
                      ...
                      <div className="u-label text-[10px] font-bold text-[#B87333] mb-1">ERC-1967 Proxy</div>
                      <div className="border-b border-[#B87333]/20 pb-2 mb-2 text-[10px] text-[#A08060] font-medium">Permanent State</div>
                      <div className="u-mono text-[9px] text-[#A08060] space-y-1">
                        <p>Slot: 0x3608...</p><p>Impl: 0xABCD...</p>
                      </div>
                    </TiltCard>
                  </div>
                  <div className="flex flex-col lg:flex-row items-center gap-4 md:gap-6 w-full lg:w-auto">
                    <div className="flex flex-col lg:flex-row items-center gap-1.5">
                      ...
                      <span className="u-mono px-2 py-1 rounded bg-black/5 text-[9px] font-bold text-[#5A5040]">delegatecall()</span>
                    ...
                    <div className="flex flex-col gap-2 w-full lg:w-64">
                      {[
                        ...
                        <div key={i} className="rounded-lg md:rounded-xl border p-3 md:p-4 flex items-center gap-3 transition-all relative"
                          style={{background:im.active?"linear-gradient(135deg,#FFF8EC,#FAF7F0)":"#FAFAFA",borderColor:im.active?"rgba(184,115,51,0.4)":"rgba(28,26,23,0.08)",opacity:im.done?0.6:1}}>
                          {im.active && (
                          ...
                          <span className="u-mono text-xs font-bold" style={{color:im.active?"#B87333":"#C0A880"}}>{im.v}</span>
                          <div className="flex-1">
                            <div className="u-mono text-[10px] font-bold" style={{color:im.active?"#5A3018":"#A08060"}}>{im.name}</div>
                            <div className="text-[9px] text-[#A08060] mt-0.5">{im.desc}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {tab==="storage" && (
              <motion.div key="stg" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-5 md:space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-8">
                  <TiltCard intensity={2} className="rounded-xl md:rounded-2xl border overflow-hidden bg-white shadow-sm flex flex-col" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                    <div className="px-4 md:px-6 py-3 border-b flex items-center justify-between bg-[#FDF9F3]" style={{borderColor:"rgba(184,115,51,0.1)"}}>
                      <span className="u-label text-[10px] font-bold text-[#A08060]">Storage Layout Visualizer</span>
                    ...
                    <div className="flex-1 min-h-[250px] md:min-h-[350px] relative"><SlotCanvas /></div>
                  </TiltCard>
                  <div className="flex flex-col gap-4">
                    <TiltCard intensity={3} className="p-5 rounded-xl md:rounded-2xl border bg-white shadow-sm" style={{borderColor:"rgba(220,38,38,0.15)"}}>
                      <div className="u-label mb-2 text-[9px] font-bold text-[#DC2626]">⚠ Storage Collision — DANGER</div>
                      <p className="text-xs md:text-sm leading-relaxed text-[#7F1D1D] mb-3">
                        Inserting a variable before existing ones shifts every subsequent slot — silently corrupting entire protocol state.
                      </p>
                      <div className="rounded-lg p-3 u-mono text-[9px] md:text-xs bg-[#FEF2F2] border border-[#FECACA] text-[#991B1B] overflow-x-auto">
                        {`// V1: Slot[0] = feeRate\n// V2: uint256 newVar; \n// ← CORRUPTS EVERYTHING`}
                      </div>
                    </TiltCard>
                    <TiltCard intensity={3} className="p-5 rounded-xl md:rounded-2xl border bg-white shadow-sm" style={{borderColor:"rgba(184,115,51,0.2)"}}>
                      <div className="u-label mb-2 text-[9px] font-bold text-[#B87333]">✓ __gap Variable — SAFE</div>
                      <p className="text-xs md:text-sm leading-relaxed text-[#6B5030] mb-3">
                        We reserve 50 storage slots at the end of each base contract. Future upgrades consume from this gap.
                      </p>
                      <div className="rounded-lg p-3 u-mono text-[9px] md:text-xs bg-[#FDF9F3] border border-[#FDE68A] text-[#92400E] overflow-x-auto">
                ...
                <div className="bg-white border border-[#B87333]/20 p-5 md:p-8 rounded-xl md:rounded-2xl text-center max-w-4xl mx-auto shadow-sm">
                  <h4 className="u-mono text-sm md:text-lg font-bold text-[#7A3A12] mb-2">_authorizeUpgrade()</h4>
                  <p className="text-xs md:text-sm text-[#6B5030] leading-relaxed">
                    In UUPS, the logic contract <strong className="text-[#1C1A17]">MUST</strong> contain this function. If you deploy an upgrade that forgets it, the Proxy becomes permanently locked forever.
                  </p>
                </div>
              </motion.div>
            )}
