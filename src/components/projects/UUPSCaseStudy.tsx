  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      [...cubes].sort((a,b)=>a.z-b.z).forEach(cube => {
        ...
        drawCube(cube.x * W, cube.y * H, cube.size * (0.55 + cube.z * 0.9), cube.rot, (0.25 + cube.z * 0.7) * pulse);
      });
...
export default function UUPSCaseStudy() {
  ...
  return (
    <>
      <style>{S}</style>
      <div className="u-root w-full overflow-hidden bg-[#FAF7F0] text-[#1C1A17]">

        {/* ── HERO HEADER ── */}
        <div className="relative w-full min-h-[40vh] bg-[#FAF7F0] overflow-hidden border-b border-[#B87333]/20 flex flex-col justify-end">
          <HeroCanvas />
          ...
          <div className="relative z-10 px-6 md:px-12 pt-24 pb-12 w-full max-w-7xl mx-auto">
            <motion.div initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} className="flex flex-wrap gap-2 mb-6">
              {["EIP-1822 UUPS", "EIP-1967 Slots", "DelegateCall", "Foundry"].map((tag,i) => (
                <span key={i} className="u-label px-3 py-1.5 text-[10px] font-bold border rounded-md"
                  style={{borderColor:"rgba(184,115,51,0.3)", color:"#7A3A12", background:"rgba(184,115,51,0.05)"}}>{tag}</span>
              ))}
            </motion.div>
            
            <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.1}}>
              <div className="u-label mb-3 text-[10px] md:text-xs font-bold text-[#A08060]">Smart Contract Architecture</div>
              <h1 className="u-serif text-4xl md:text-6xl lg:text-7xl font-black leading-[1.05] text-[#1C1A17] mb-6">
                Mutable <br className="hidden md:block" />
                <span className="text-[#B87333]">Infrastructure.</span>
              </h1>
              <p className="text-sm md:text-base leading-relaxed text-[#6B5030] max-w-2xl font-medium">
            ...
            <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.2}} className="flex flex-wrap gap-4 mt-8">
              <Link href="https://github.com/NexTechArchitect/uups-protocol-config" target="_blank"
                className="u-label inline-flex items-center gap-2 px-6 py-4 text-xs font-bold rounded-xl transition-all hover:-translate-y-1"
        ...
        <div className="border-b bg-white" style={{borderColor:"rgba(184,115,51,0.15)"}}>
          <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 divide-x divide-[#B87333]/10">
            {[
              ...
              <div key={i} className="p-6 md:p-8 text-center md:text-left">
                <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:0.2+i*0.05}}>
                  <div className="u-serif text-3xl md:text-4xl font-black text-[#B87333] mb-2 leading-none">{s.v}</div>
                  <div className="u-label text-[10px] md:text-xs text-[#7A3A12] font-bold mb-1">{s.sub}</div>
                  <div className="text-[10px] md:text-xs text-[#A08060]">{s.note}</div>
                </motion.div>
              </div>
            ))}
          </div>
        ...
        <div className="sticky top-0 z-30 bg-[#FAF7F0]/90 backdrop-blur-md border-b" style={{borderColor:"rgba(184,115,51,0.15)"}}>
          <div className="max-w-7xl mx-auto px-4 md:px-12 pt-4 pb-0 flex overflow-x-auto hide-scroll gap-2 md:gap-4">
            {TABS.map(t => (
              <button key={t.id} onClick={()=>setTab(t.id)}
                className="relative u-label px-4 py-4 text-[10px] md:text-xs font-bold whitespace-nowrap transition-colors"
        ...
        {/* ── CONTENT AREA ── */}
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 md:py-16 min-h-[50vh]">
          <AnimatePresence mode="wait">
            ...
            {tab==="overview" && (
              <motion.div key="ov" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-8">
                    ...
                    <table className="w-full text-left min-w-[600px]">
                      <thead style={{background:"#FDF9F3", borderBottom:"1px solid rgba(184,115,51,0.1)"}}>
                        <tr>
                          {["Feature", "Standard Contract", "UUPS Proxy Standard"].map((h,i) => (
                            <th key={i} className="u-label px-6 py-4 text-[10px] md:text-xs" style={{color:i===2?"#7A3A12":"#A08060", fontWeight:800}}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#B87333]/10">
                        {[
                          ...
                          { f: "Gas Cost", e: "Lower (Direct execution)", a: "Minimal (DelegateCall overhead)" },
                          { f: "Bug Fixes", e: "Impossible (Must redeploy)", a: "Possible (Upgrade implementation)" },
                          { f: "Admin Check", e: "N/A", a: "Secure via _authorizeUpgrade()" },
                        ].map((row,i) => (
                          <tr key={i} className="hover:bg-[#FAF7F0]/50 transition-colors">
                            <td className="px-6 py-4 text-sm font-bold text-[#1C1A17] whitespace-nowrap">{row.f}</td>
                            <td className="px-6 py-4 text-sm text-[#A08060]">{row.e}</td>
                            <td className="px-6 py-4 text-sm text-[#7A3A12] font-semibold">{row.a}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <TiltCard intensity={3} className="p-6 md:p-8 rounded-2xl border bg-white shadow-sm" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                    <div className="u-label mb-3 text-[10px] font-bold" style={{color:"#B09070"}}>// Why UUPS over Transparent</div>
                    <h3 className="text-lg font-black text-[#1C1A17] mb-3">Eliminating Admin Checks</h3>
                  ...
                  <TiltCard intensity={3} className="p-6 md:p-8 rounded-2xl border bg-white shadow-sm" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                    <div className="u-label mb-4 text-[10px] font-bold" style={{color:"#B09070"}}>// Repository Scope</div>
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={i} className="flex items-start gap-4">
                          <span className="u-mono w-6 h-6 shrink-0 rounded-md flex items-center justify-center font-black text-xs" style={{background:"rgba(184,115,51,0.1)", color:"#7A3A12"}}>{i+1}</span>
                          <div>
                            <h4 className="text-sm font-bold text-[#1C1A17]">{item.title}</h4>
            ...
            {tab==="architecture" && (
              <motion.div key="arch" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-8">
                ...
                <h3 className="u-label text-xs font-bold text-[#A08060] mb-6 text-center md:text-left">The DelegateCall Flow</h3>
                
                <div className="rounded-3xl border p-6 md:p-12 bg-white shadow-sm flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-12" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                  
                  {/* User -> Proxy */}
                  <div className="flex flex-col md:flex-row items-center gap-6 w-full lg:w-auto">
                    <TiltCard intensity={5} className="rounded-2xl border p-6 text-center w-full md:w-32 shadow-md shrink-0" style={{background:"#FDF9F3",borderColor:"rgba(28,26,23,0.1)"}}>
                      <div className="text-4xl mb-3">👤</div>
                      <div className="u-label text-[10px] font-bold text-[#A08060]">User</div>
                    </TiltCard>

                    <div className="flex flex-col md:flex-row items-center gap-2">
                       <div className="w-1 h-8 md:w-16 md:h-1 bg-gradient-to-b md:bg-gradient-to-r from-[#B87333]/20 to-[#B87333]/70 rounded-full" />
                       <span className="u-mono px-3 py-1 rounded-lg text-[10px] font-bold" style={{background:"rgba(184,115,51,0.1)",color:"#7A3A12"}}>call()</span>
                       <div className="w-1 h-8 md:w-16 md:h-1 bg-gradient-to-b md:bg-gradient-to-r from-[#B87333]/70 to-[#B87333]/20 rounded-full" />
                    </div>

                    <TiltCard intensity={5} className="rounded-2xl border p-6 md:p-8 w-full md:w-56 shrink-0 relative overflow-hidden"
                      ...
                      <div className="absolute top-0 right-0 w-24 h-24 bg-[#B87333]/10 rounded-full blur-[20px] pointer-events-none" />
                      <div className="u-label text-xs font-black text-[#B87333] mb-2">ERC-1967 Proxy</div>
                      <div className="border-b border-[#B87333]/20 pb-3 mb-3 text-xs text-[#A08060] font-medium">Permanent State</div>
                      <div className="u-mono text-[10px] text-[#A08060] space-y-1">
                        <p>Slot: 0x3608...</p>
                        <p>Impl: 0xABCD...</p>
                      </div>
                    </TiltCard>
                  ...
                  <div className="flex flex-col lg:flex-row items-center gap-6 w-full lg:w-auto">
                    <div className="flex flex-col lg:flex-row items-center gap-2">
                       <div className="w-1 h-8 lg:w-16 lg:h-1 bg-gradient-to-b lg:bg-gradient-to-r from-[#1C1A17]/10 to-[#1C1A17]/40 rounded-full" />
                       <span className="u-mono px-3 py-1 rounded-lg text-[10px] font-bold" style={{background:"rgba(28,26,23,0.05)",color:"#5A5040"}}>delegatecall()</span>
                       <div className="w-1 h-8 lg:w-16 lg:h-1 bg-gradient-to-b lg:bg-gradient-to-r from-[#1C1A17]/40 to-[#1C1A17]/10 rounded-full" />
                    </div>
                    ...
                    <div className="flex flex-col gap-3 w-full lg:w-64">
                      {[
                        ...
                        <div key={i} className="rounded-xl border p-4 flex items-center gap-4 transition-all relative"
                          style={{
                          ...
                          {im.active && (
                            <span className="absolute -left-2 top-1/2 -translate-y-1/2 flex h-4 w-4">
                              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#B87333] opacity-60"></span>
                              <span className="relative inline-flex rounded-full h-4 w-4 bg-[#B87333] border-2 border-white"></span>
                            </span>
                          )}
                          <span className="u-mono text-sm font-black" style={{color: im.active ? "#B87333" : "#C0A880"}}>{im.v}</span>
                          <div className="flex-1">
                            <div className="u-mono text-xs font-bold" style={{color: im.active ? "#5A3018" : "#A08060"}}>{im.name}</div>
                            <div className="text-[10px] text-[#A08060] mt-0.5">{im.desc}</div>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {tab==="storage" && (
              <motion.div key="stg" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}} className="space-y-8">
                
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  {/* Left: Canvas */}
                  <TiltCard intensity={2} className="rounded-3xl border overflow-hidden bg-white shadow-sm flex flex-col" style={{borderColor:"rgba(184,115,51,0.15)"}}>
                    <div className="px-6 py-4 border-b flex items-center justify-between bg-[#FDF9F3]" style={{borderColor:"rgba(184,115,51,0.1)"}}>
                      <span className="u-label text-xs font-bold text-[#A08060]">Storage Layout Visualizer</span>
                  ...
                  <div className="flex flex-col gap-6">
                    <TiltCard intensity={3} className="p-6 rounded-3xl border bg-white shadow-sm" style={{borderColor:"rgba(220,38,38,0.15)"}}>
                      <div className="u-label mb-3 text-[10px] font-bold text-[#DC2626]">⚠ Storage Collision — DANGER</div>
                      <p className="text-sm leading-relaxed text-[#7F1D1D] mb-4">
                      ...
                      <div className="rounded-xl p-4 u-mono text-xs bg-[#FEF2F2] border border-[#FECACA] text-[#991B1B]">
                        {`// V1: Slot[0] = feeRate\n// V2: uint256 newVar; \n// ← CORRUPTS EVERYTHING`}
                      </div>
                    </TiltCard>

                    <TiltCard intensity={3} className="p-6 rounded-3xl border bg-white shadow-sm" style={{borderColor:"rgba(184,115,51,0.2)"}}>
                      <div className="u-label mb-3 text-[10px] font-bold text-[#B87333]">✓ __gap Variable — SAFE</div>
                      <p className="text-sm leading-relaxed text-[#6B5030] mb-4">
                      ...
                      <div className="rounded-xl p-4 u-mono text-xs bg-[#FDF9F3] border border-[#FDE68A] text-[#92400E]">
                ...
                <div className="bg-[#FAF7F0] border border-[#B87333]/20 p-8 rounded-3xl text-center max-w-4xl mx-auto shadow-inner">
                   <h4 className="u-mono text-lg font-black text-[#7A3A12] mb-3">_authorizeUpgrade()</h4>
                   <p className="text-sm text-[#6B5030] leading-relaxed">
                     In UUPS, the logic contract <strong className="text-[#1C1A17]">MUST</strong> contain this function. If you deploy an upgrade that forgets to include it, the Proxy becomes permanently locked and can never be upgraded again. It serves as the sole, highly-secured gatekeeper of the protocol's lifecycle.
                   </p>
                </div>
